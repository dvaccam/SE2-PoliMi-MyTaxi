...
/**
 * The class is the manager of the enviroment of various components (known by the IDs).
 * When components want to register to some JNDINameEnviroment (using some ID), they notify so to this manager.
 * Thus, the manager knows to which JNDINameEnv refer for calls on the components (based on the ID of the call)
*/
@Service
public class ComponentEnvManagerImpl
    ...
    //JNDINameEnviroment relative that name
    //The JNDINameEnviroment is just an interface that allows access to relevant info of the enviroment of a registered object;
    //such object must implement the interface. So a variale of typ JNDINameEnviroment can be seen both as the object and its enviroment
    ...
    //Get environment of the current invocation of the container
    public JndiNameEnvironment getCurrentJndiNameEnvironment() {
        JndiNameEnvironment desc = null;
        ComponentInvocation inv = invMgr.getCurrentInvocation();
        if (inv != null) {
            if (inv.componentId != null) {
                desc = getJndiNameEnvironment(inv.componentId); //ADDITIONAL PROBLEM!!!
    ...
    //Depending on the pertinent scope, the JNDI bindings relative to the enviroment are added.
    //First get the bindings in a Collection, and then bind them to the namespace
    ...
    //More specific than the previous one. Does not use all the info in the JNDIEnv but the one passed in the parameters
    ...
    //Create binding for the things associted to tha object (?)
    //TODO
